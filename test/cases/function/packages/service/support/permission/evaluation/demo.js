...
async function testApiCall(url, token, tokenType, testName) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`✅ ${testName}: 成功 (${data.list?.length || 0} 项)`);
      if (data.list && data.list.length > 0 && data.list[0].permission) {
        console.log(
          `   权限信息: 读${data.list[0].permission.hasReadPer} 写${data.list[0].permission.hasWritePer} 管理${data.list[0].permission.hasManagePer}`
        );
      }
...
async function smartTest() {
  ...
  const apiEndpoints = [
    ...
    { name: '评估数据集', path: '/api/core/evaluation/dataset/list' },
