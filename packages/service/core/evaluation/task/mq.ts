...
evaluationItemQueue.on('waiting', async (job) => {
  try {
    const evalItemId = job?.data?.evalItemId;

    addLog.debug('[Evaluation] Item job waiting in queue', {
      jobId: job?.id,
      evalId: job?.data?.evalId,
      evalItemId
    });

    // Update status to queuing when job enters waiting state
    if (evalItemId) {
    ...
    addLog.warn('[Evaluation] Item job waiting (could not get job data)', {
      jobId: job?.id,
      error
    });
  }
